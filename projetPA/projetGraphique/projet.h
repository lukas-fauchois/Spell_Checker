...
typedef struct cell* ptr_cellule;
struct cell{int val;ptr_cellule suivant;};

typedef struct{GtkWidget *p_texte; Lettre* dico;}Test;

void ajout_tete(struct cell**, char);

void suppression_tete(struct cell**);

void supprimer_liste(struct cell**);
...
gchar* correction_texte(Lettre*, char*, int);
